...
class MegaCloud {
  ...
  async extract(videoUrl: URL) {
    try {
      ...
      if (!srcsData) {
        throw createHttpError(500, "Failed to fetch sources");
      }
      ...
      if (!text) {
        ...
        //   500
        // );
        throw createHttpError(500, "Couldn't fetch script to decrypt resource");
      }
      ...
      try {
        ...
        //   500
        // );
        throw createHttpError(500, "Failed to decrypt resource");
      }
    } catch (err) {
      // console.log(err);
      throw err;
